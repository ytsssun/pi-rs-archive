...
**Current integration boundary:** pi-rs has a custom headless CLI plus an experimental upstream CLI path. The intended product boundary is unchanged upstream `coding-agent` with its `agent` implementation replaced by a Rust runtime behind a JS-compatible facade. The custom path is transitional and should not define the long-term product. The existing drive module is not an Agent-compatible object. Rust also owns selective/all user queue clearing in the experimental Agent adapter and explicit queued continuation admission and validated last-error retry projection after a settled experimental Agent run; journals remain unchanged by that projection. A separate experimental Agent adapter now supports unchanged upstream CLI new/resume through the original provider stack against a local HTTP fixture; see the review for its substantial remaining limits. See [upstream package boundaries](upstream-package-boundaries.md) and [executed core replacement review](core-replacement-review.md) for the negative injection experiment and next acceptance. Historical sections below contain stale ownership details and must not be treated as current verification.
